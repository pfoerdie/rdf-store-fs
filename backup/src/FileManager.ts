...
export default class FileManager {
  ...
  #files = new Map<Token, FileSpec | Promise<FileSpec>>()

  async #open(fileName: Token): Promise<FileSpec> {
    const file = this.#files.get(fileName)
    if (file) return file

    // let file = 
    //     if (!this.#files.has(fileName)) this.#files.set(fileName, { name: fileName })
    //     const file = this.#files.get(fileName) as FileSpec
    // if (!file.handle)
